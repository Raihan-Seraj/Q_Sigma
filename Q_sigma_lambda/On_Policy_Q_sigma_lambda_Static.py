...
def behaviour_policy_Boltzmann(theta, tau, nA):
    def policy_fn(observation):
        ...
        policy = np.true_divide(  np.exp(exp_tau),   np.sum(np.exp(exp_tau), axis=0)  )
        A = policy
...
def q_sigma_lambda_off_policy_decaying_sigma(env, theta, num_episodes, discount_factor=1.0, epsilon=0.1, epsilon_decay=1.0, alpha=0.1, lambda_param=0.8):
	...
	for i_episode in range(num_episodes):
		...
		print ("Episode Number, Q (sigma, lambda) On Policy, Decaying Sigma:", i_episode)
		...
		off_policy = epsilon_greedy_policy(theta, epsilon * epsilon_decay**i_episode, env.action_space.n)
		state = env.reset()
...
def take_average_results(experiment,num_experiments,num_episodes,env,theta):
	...
	for i in range(num_experiments):
		...
		np.save('/home/raihan/Desktop/Final_Project_Codes/cliff_walking_results/Q_sigma_lambda/'  + 'Qsigmalambda_onpolicy_static_reward' + '.npy',average_reward)
		np.save('/home/raihan/Desktop/Final_Project_Codes/cliff_walking_results/Q_sigma_lambda/'  + 'Qsigmalambda_onpolicy_static__error' + '.npy',average_error)
...
def main():
	...
	num_episodes = 1000
	smoothing_window = 1
	avg_cum_reward,avg_cum_error=take_average_results(q_sigma_lambda_off_policy_decaying_sigma,num_experiments,num_episodes,env,theta)
